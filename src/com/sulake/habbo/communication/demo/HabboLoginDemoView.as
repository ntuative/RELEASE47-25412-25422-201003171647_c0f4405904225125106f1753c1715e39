package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_692:String = "InitConnection";
      ...
      private var var_349:HabboCommunicationDemo;
      ...
      {
         var_349.windowManager.removeWindow("habbo_login_window");
      }
      ...
      private function onParsedTicketSuccess(param1:Event) : void
      {
         ...
         var_349.shockwaveClientUrl = var_115.shockwaveClientUrl;
         var_349.ssoTicket = var_115.ticket;
         var_115.removeEventListener(SSOTicket.const_290,onParsedTicketSuccess);
         var_115.removeEventListener(SSOTicket.const_236,onParsedTicketFailure);
         var_115 = null;
         dispatchEvent(new Event(const_692));
      }
      ...
      {
         var _loc1_:XmlAsset = (var_349 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         _window = var_349.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_389,HabboWindowStyle.const_41,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = var_349.windowManager.buildFromXML(_loc2_) as IWindowContainer;
      ...
      private function initSSOTicket() : void
      {
         ...
         if(false)
         {
            _loc1_ = var_349.habboConfiguration.getKey("url.prefix",_loc1_);
            _loc1_ = _loc1_.replace("http://","");
            _loc1_ = _loc1_.replace("https://","");
         }
         var _loc2_:IHabboWebLogin = var_349.habboCommunication.habboWebLogin(name,password);
         var_115 = new SSOTicket(var_349.assets,_loc2_,_loc1_);
         var_115.addEventListener(SSOTicket.const_290,onParsedTicketSuccess);
         var_115.addEventListener(SSOTicket.const_236,onParsedTicketFailure);
      }
