      ...
      public function processEvent(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            case RoomSessionQueueEvent.const_412:
               ...
               {
                  case RoomSessionQueueEvent.const_922:
                     _loc3_ = "null";
                     break;
                  case RoomSessionQueueEvent.const_863:
                     _loc3_ = "null";
               }
               if(_loc3_ == null)
               {
               ...
               if(_loc7_.length > 1)
               {
                  if(_loc6_ && _loc2_.queueTypes.indexOf(RoomSessionQueueEvent.const_796) != -1)
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_796) + 1;
                     _loc9_ = true;
                  }
                  else
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1229) + 1;
                  }
               }
               else
               {
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            ...
            case RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_863);
               break;
            case RoomWidgetRoomQueueMessage.const_571:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_922);
               break;
            case RoomWidgetRoomQueueMessage.const_504:
               if(false)
               {
                  _container.catalog.openCatalogPage(CatalogPageName.const_239,true);
               }
         }
         return null;
      }
