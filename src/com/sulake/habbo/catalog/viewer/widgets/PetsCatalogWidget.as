package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_423 >= var_312.length)
         {
            return "";
         }
         var petColor:IPetColor = var_312[var_423] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_603[var_1546] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_423 = param1.index;
         if(var_423 < 0 || var_423 > var_312.length)
         {
            var_423 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_577.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_158,onTextWindowEvent);
         var_577 = new Map();
         ...
         var_896 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_312 = [];
         var_603 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_896);
               if(_loc11_)
               {
                  var_603 = _loc11_.breeds;
                  var_312 = _loc11_.colors;
                  if(var_312 == null)
                  {
                     var_312 = [];
                  }
         ...
         while(_loc6_ < var_603.length)
         {
            ...
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_896,_loc12_.id),getRaceLocalizationKey(var_896,_loc12_.id)));
            _loc6_++;
         }
         var_577.add(var_896,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_930,onApproveNameResult);
         _loc5_ = var_577.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_312)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_577.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_423 >= 0 && var_423 < var_312.length)
         {
            _loc7_ = var_312[var_423] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_603[var_1546] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_896,_loc3_.id,_loc2_,AvatarScaleType.const_52);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_331,AvatarAction.const_714);
