package com.sulake.habbo.notifications
{
   ...
   public class HabboNotificationItemView implements IUpdateReceiver
   {
      ...
      public static const const_735:int = 70;
      ...
      public function HabboNotificationItemView(param1:IAsset, param2:IHabboWindowManager, param3:Map, param4:Map, param5:IHabboToolbar)
      {
         ...
         if(_loc7_ != null)
         {
            var_1305 = 0 - _loc7_.rectangle.bottom;
         }
         else
         {
            var_1305 = 15;
         }
         var_1536 = _window.height;
         var_595 = 4;
         var_419 = 0;
         var_35 = const_347;
         var_189 = param5;
      ...
      private function setNotificationText(param1:String) : void
      {
         ...
         _window.height = 0;
         _loc2_.text = param1;
         _loc2_.height = _loc2_.textHeight + var_1305;
         if(_window.height < var_1536)
         {
            _window.height = var_1536;
         }
      }
      ...
      public function update(param1:uint) : void
      {
         ...
         switch(var_35)
         {
            ...
            case const_380:
               var_877 += param1;
               _loc2_ = Number(var_877) / Number(var_418["time_fade_in"]);
               if(var_877 > int(var_418["time_fade_in"]))
               {
                  startDisplay();
               }
               adjustBlend(_loc2_);
            ...
            case const_777:
               var_1059 += param1;
               if(var_1059 > int(var_418["time_display"]) && !var_1306)
               {
                  startFadeOut();
               }
               break;
            case const_778:
               var_878 += param1;
               _loc3_ = 1 - Number(var_878) / Number(var_418["time_fade_out"]);
               adjustBlend(_loc3_);
               if(var_878 > int(var_418["time_fade_in"]))
               {
                  startIdling();
               }
         }
      ...
      public function reposition(param1:int = -1) : void
      {
         ...
         var _loc3_:int = var_189.orientation == "TOP" ? int(const_346) : 0;
         var _loc4_:int = var_189.orientation == "BOTTOM" ? int(_loc2_.height - const_346) : int(_loc2_.height);
         var _loc5_:int = var_189.orientation == "LEFT" ? int(const_346) : 0;
         var _loc6_:int = var_189.orientation == "RIGHT" ? int(_loc2_.width - const_346) : int(_loc2_.width);
         if(false)
         {
            if(false)
            {
               _window.x = _loc5_ + const_351;
            }
            else
            {
               _window.x = _loc6_ - 0 - const_351;
            }
         }
         else if(false)
         {
         ...
         if(false)
         {
            if(false)
            {
               _window.y = _loc3_ + var_595;
            }
            else
            {
               _window.y = _loc4_ - 0 - var_595;
            }
         }
         else if(false)
         {
      ...
      {
         if(param1.badgeId != var_221.style.iconSrc)
         {
            return;
         }
