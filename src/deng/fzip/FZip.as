package deng.fzip
{
   ...
   public class FZip extends EventDispatcher
   {
      ...
      protected function progressHandler(param1:Event) : void
      {
         ...
         {
            close();
            if(!hasEventListener(FZipErrorEvent.const_703))
            {
               throw e;
            }
            dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_703,e.message));
         }
      }
      ...
      public function loadBytes(param1:ByteArray) : void
      {
         if(!var_90 && parseFunc == parseIdle)
         {
            ...
            {
               dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_703,"EOF"));
            }
         }
      }
      ...
      private function parseLocalfile(param1:IDataInput) : Boolean
      {
         if(var_484.parse(param1))
         {
            ...
            if(false)
            {
               var_129["null"] = var_484;
            }
            dispatchEvent(new FZipEvent(FZipEvent.const_1333,var_484));
            var_484 = null;
